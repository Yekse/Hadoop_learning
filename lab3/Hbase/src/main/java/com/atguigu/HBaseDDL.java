...
public class HBaseDDL {
    ...
    public static void listTable() throws IOException {
        ...
        for (TableName tb: tableNameList) {
    ...
    /**
     * 获取表格描述
     * @return tableDescriptor 表示指定表格的描述
     */
    public static TableDescriptor describeTable(String namespace , String tableName)
            throws IOException {
        // 获取 admin
        Admin admin = connection.getAdmin();
        TableDescriptor tableDescriptor = admin.getDescriptor(TableName.valueOf(namespace,
                tableName));
     ...
     * @return true 表示清空成功
     */
    public static boolean emptyTable(String namespace ,String tableName) throws IOException {
        // 1. 判断表格是否存在
        if (!isTableExists(namespace,tableName)){
            System.out.println("表格不存在 无法删除");
            return false;
        ...
        // 3. 调用相关的方法删除表格
        try {
            // HBase 删除表格之前 一定要先标记表格为disable
            TableName tableName1 = TableName.valueOf(namespace, tableName);
            admin.disableTable(tableName1);
            admin.truncateTable(tableName1, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
     ...
     * @param family 列族名称
     */
    public static void addColumnFamily(String namespace , String tableName, String family)
            throws IOException {
        // 1. 判断表格是否存在
        if (!isTableExists(namespace,tableName)){
            System.out.println("表格不存在 无法添加");
            return;
        }
        // 2. 获取 admin
        Admin admin = connection.getAdmin();
     ...
     * @param family 列族名称
     */
    public static void deleteColumnFamily(String namespace , String tableName, String family)
            throws IOException {
        // 1. 判断表格是否存在
        if (!isTableExists(namespace,tableName)){
            System.out.println("表格不存在 无法删除");
        ...
        Admin admin = connection.getAdmin();
        TableName tablename=TableName.valueOf(tableName);
        admin.deleteColumnFamily(tablename, Bytes.toBytes(family));
        // 4. 关闭 admin
        admin.close();
    }
...
//
        ...
        createTable("","students","info");
        listTable();
//       addColumnFamily("", "students", "msg");
 //       deleteColumnFamily("", "students", "msg");
